...
def _get_veterinaria(request):
    """
    Auxiliar robusto para obtener la veterinaria activa según el contexto, 
    middleware tenant o perfil de usuario.
    """
    # 1. Atributo inyectado por TenantMiddleware
    if hasattr(request, 'veterinaria') and request.veterinaria:
        return request.veterinaria
    
    # 2. Atributo en el perfil del usuario
